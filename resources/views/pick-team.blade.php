                        ...
                        <div style="grid-template-columns: 1fr 1fr 1fr; " class="d-grid gap-5 p-5 align-self-center ">
                            @foreach ($teams as $team) 
                                <div class="card" style="width: 20rem;">
                                    <div class="card-body">
                                        <h5 class="card-title">{{ strtoupper("Candidate ".$team->pair->value) }}</h5>
                                         <p class="card-text">Our Vision</p>
                                        <p class="card-text"> {{ $team->vision }}</p>
                                        <p class="card-text">Our Mission</p>
                                        <p class="card-text"> {{ $team->mission }}</p>
                                        <form action="{{ url('/pick/' . $team->id) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="btn btn-outline-success">Pick</button>
                                        </form>
                                    </div>
                                </div>   
                            @endforeach
